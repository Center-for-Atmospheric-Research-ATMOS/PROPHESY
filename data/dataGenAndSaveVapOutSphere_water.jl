...
P_H2O = 1000.0*2.0e-5*0.025; # 2.0e-6* # can not explain the scaling factor here
ρ_gas = (P_H2O*P_gas*M_water/(R_gas*T_gas))*((μ0./r_gas).^2); # assume that the gas concentration profile is decreasing inversly proportional to the distance to the sample (the amount of mater crossing the surface of the cylinder is the same that crossing a bigger virtual cylinder)
...
xc = kc*σx;
yc = 50.0 # 70.0 # 75.0; # 5.0*σy; # 6.0*σy #WARNING: this value accounts for the 
...
for j in 1:Ndata # can potentially multi-thread this loop: but need to sync before writing files
    ...
    ## geometry factors
    ##
    local H_deom,_,H_geom,_,_,_,_,al_liq = alignmentParameterSphere(bp,r,φ,θ,x0,y0,z0,μ0,λe[j])
    ...
    ## gas phase signal
    ##
    local H_deom_gas,_,H_geom_gas,_,_,_,_,al_gas = alignmentParameterSphere(bp,r_gas,φ_gas,θ_gas,x0,y0,z0,μ0,λe[j])
