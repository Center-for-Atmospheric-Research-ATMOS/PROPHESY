...
"""
function F_gaussian_un(z::Array{Cdouble,1},y::Array{Cdouble,1},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1})
    Nke = length(y);
    M = size(γ_H,2);
    M_d = length(γ_D);
    W_i = (1.0/4.0)./dropdims(sum(((1.0./γ).*γ_H).^2,dims=1),dims=1) # should be of dimension M
    # println(summary((1.0./γ)))
    # println(summary((γ_H.^2)*(z[Nke+1:Nke+M].^2)))
    # sum(((z[1:Nke]-y)./γ).^2) + sum((1.0./γ)*((γ_H.^2)*(z[Nke+1:Nke+M].^2))) + sum((1.0./γ_D.^2).*z[Nke+M+1:end].^2)
    sum(((z[1:Nke]-y)./γ).^2) + (1.0./γ)'*((γ_H.^2)*(z[Nke+1:Nke+M].^2)) + sum((1.0./γ_D.^2).*z[Nke+M+1:end].^2)
end
...
"""
function G_gaussian_un(x::Array{Cdouble,1})
    val = 0.0
    if any(x.<0.0)
        val = Inf
    end
...
"""
function F_convex_conjugate_gaussian_un(u::Array{Cdouble,1},y::Array{Cdouble,1},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1})
    Nke = length(y);
    M = size(γ_H,2);
    M_d = length(γ_D);
    W_i = (1.0/4.0)./dropdims(sum(((1.0./γ).*γ_H).^2,dims=1),dims=1) # should be of dimension M
    sum(u[1:Nke].*(y+0.25(γ.^2).*u[1:Nke])) + sum(W_i.*u[Nke+1:Nke+M]) + sum(0.25(γ_D.^2).*u[Nke+M+1:end])
end

function G_convex_conjugate_gaussian_un(u::Array{Cdouble,1})
    val = 0.0
    if any(u.>0.0)
        val = Inf
    end
...
"""
function prox_F_conj_gaussian_un(x::Array{Cdouble},y::Array{Cdouble,1}, σ::Cdouble,γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1})
    Nke = length(y);
    M = size(γ_H,2);
    M_d = length(γ_D);

    W_i = (1.0/4.0)./dropdims(sum(((1.0./γ).*γ_H).^2,dims=1),dims=1)
    u_all = zeros(Cdouble,Nke+M+M_d);
...
"""
function prox_G_gaussian_un(x::Array{Cdouble})
...
"""
function alg2_cp_gaussian_un(x0::Array{Cdouble,1},s0::Array{Cdouble,1},y::Array{Cdouble,1},A::Array{Cdouble,2},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},W_stop::Array{Cdouble,1};tau0::Cdouble=1.0,Niter::Int64=100,r_n_tol::Cdouble=1.0e-6,r_y_tol::Cdouble=0.5)
    ...
    for i in 1:Niter
        # dual step
        sn = prox_F_conj_gaussian_un(sn+sigman*A*xn_bar,y,sigman,γ,γ_H,γ_D);
        # primal step
        xn_pre = xn; # memory (TODO: check that it does what I think it does)
        xn     = prox_G_gaussian_un(xn-taun*A'*sn);
...
function alg2_cp_gaussian_un_no_mem(x0::Array{Cdouble,1},s0::Array{Cdouble,1},y::Array{Cdouble,1},A::Array{Cdouble,2},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},W_stop::Array{Cdouble,1};tau0::Cdouble=1.0,Niter::Int64=100,r_n_tol::Cdouble=1.0e-6,r_y_tol::Cdouble=0.5)
    # init algorithm's parameters
    N = length(x0);
    if (N!=size(A,2))
        throw("alg2_chpo: operator A and state x0 do not have compatible dimensions")
    ...
    dX_W = copy(x0);
    F_ALL[1]      = F_gaussian_un(A*xn,y,γ,γ_H,γ_D)
    G_ALL[1]      = G_gaussian_un(xn);
    F_STAR_ALL[1] = F_convex_conjugate_gaussian_un(sn,y,γ,γ_H,γ_D);
    G_STAR_ALL[1] = G_convex_conjugate_gaussian_un(-A'*sn);
    ...
    for i in 1:Niter
        ...
        F_ALL[i+1]      = F_gaussian_un(A*xn,y,γ,γ_H,γ_D)
        G_ALL[i+1]      = G_gaussian_un(xn);
        F_STAR_ALL[i+1] = F_convex_conjugate_gaussian_un(sn,y,γ,γ_H,γ_D);
        G_STAR_ALL[i+1] = G_convex_conjugate_gaussian_un(-A'*sn);
