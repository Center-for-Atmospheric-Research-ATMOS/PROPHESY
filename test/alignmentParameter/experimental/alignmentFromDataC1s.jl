...
for idx_file in eachindex(data_filesC1s)
    ...
    for i in 1:Ndata
        ...
        α_al_noise[idx]      = α_al_noise[idx]/(κ_units*σ_C1s_exp(convert(Cdouble,df_Eph[!,photon_sym][i]))*dictPeak[plot_sym][!,ph_flu_sym][1]*dKe)
        # α_al_noise[idx]      = α_al_noise[idx]/(κ_units*σ_C1s_exp(convert(Cdouble,df_Eph[!,photon_sym][i]))*dictPeak[plot_sym][!,ph_flu_sym][1])
        # α_al_noise[idx]      = α_al_noise[idx]/(κ_units*σ_C1s_exp(convert(Cdouble,df_Eph[!,photon_sym][i]))*dictPeak[plot_sym][!,ph_flu_sym][1]*ΔtT)
