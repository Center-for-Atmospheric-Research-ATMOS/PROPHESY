...
for idx_file in 1:length(data_filesO1sS2p)
    ...
    dictAllData,df_fit,Ndata = dataAndFit_xlsx2df_missing(string(data_folderO1sS2p,fileName,".xlsx"); regData=regData, regFit=regFit, regEph=regEph, sortBy=sortBy, thenSortBy=thenSortBy);
    ...
    for i in 1:Ndata
        ...
        local σ_all = Curve1_tot # dictAllData[plot_sym].Curve1 
