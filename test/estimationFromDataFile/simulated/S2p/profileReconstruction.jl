...
d0 = 18.5e-3 # 15.0e-3 # 2.5e-3 #  [μm] maximum depth
N0 = findlast(r.-μ0.<=-d0);
N = Nr-N0;
...
Γd = (N/NdataS2p)*(σd^2)*Γprior[N0-1:end-1,N0-1:end-1];  # scale the a priori strength with the quantity of data, so that it is possible to compare the results
Γd_inv = inv(Γd);
