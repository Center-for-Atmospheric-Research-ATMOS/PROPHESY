...
for k in 1:Nnoise
    ...
    ρ_all = samplePosteriorEntropy(zeros(Cdouble,Nr),Γsqrt,p0*ones(Cdouble,Ns),y_data[k,:],ΓIinv[:,:,k],H_better,ρA_1;Ns=Ns)
    ...
    for i in 1:Ns+1
        ...
        EpriorEntropy[i] = sum(ρ_all[i,:]-ρA_1 - ρ_all[i,:].*log.(ρ_all[i,:]./ρA_1));
    end
