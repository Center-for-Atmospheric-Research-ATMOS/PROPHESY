...
σν_exp = zeros(Cdouble,5,length(d_c1s_Ek[:,1]));
...
for i in 1:5
   ...
   x_dist_3 = (τt[i,3]/σt[i,3])*exp.(-0.5*((d_c1s_Ek[:,(2i)-1].-μt[i,3])/σt[i,3]).^2);
   max_xx = maximum(x_dist_1+x_dist_2+x_dist_3);
   σν_exp[i,:] = (σ_cs_orb(ħν_exp[i],"C1s")/max_xx)*(x_dist_1+x_dist_2+x_dist_3)
end

wsXPS = XPSsetup(ħν_exp,Fν_exp,μKe_exp,T_exp,Be_exp,σν_exp;α_exp=1.0);
...
σ_noise = 0.0
IA_1 = IA_1_clean + σ_noise*randn(wsXPS.Nke*wsXPS.Nbe);
IA_2 = IA_2_clean + σ_noise*randn(wsXPS.Nke*wsXPS.Nbe);
IA_3 = IA_3_clean + σ_noise*randn(wsXPS.Nke*wsXPS.Nbe);

figure();
for i in 1:wsXPS.Nν
   scatter(wsXPS.Be[i,:],IA_1[(i-1)*wsXPS.Nbe+1:i*wsXPS.Nbe]) # ,color="tab:blue")
   plot(wsXPS.Be[i,:],IA_1_clean[(i-1)*wsXPS.Nbe+1:i*wsXPS.Nbe]) # ,color="tab:blue")
end
xlabel("B\$_e\$ [a.u.]")
ylabel("PE signal [a.u.]")
xlim(minimum(wsXPS.Be),maximum(wsXPS.Be))
title("simulated spectra")
ylim(0.0,1.1maximum(IA_1))


figure();
for i in 1:wsXPS.Nν
   scatter(wsXPS.Be[i,:],IA_2[(i-1)*wsXPS.Nbe+1:i*wsXPS.Nbe]) # ,color="tab:blue")
   plot(wsXPS.Be[i,:],IA_2_clean[(i-1)*wsXPS.Nbe+1:i*wsXPS.Nbe]) # ,color="tab:blue")
end
xlabel("B\$_e\$ [a.u.]")
ylabel("PE signal [a.u.]")
xlim(minimum(wsXPS.Be),maximum(wsXPS.Be))
title("simulated spectra")
ylim(0.0,1.1maximum(IA_2))


figure();
for i in 1:wsXPS.Nν
   scatter(wsXPS.Be[i,:],IA_3[(i-1)*wsXPS.Nbe+1:i*wsXPS.Nbe]) # ,color="tab:blue")
   plot(wsXPS.Be[i,:],IA_3_clean[(i-1)*wsXPS.Nbe+1:i*wsXPS.Nbe]) # ,color="tab:blue")
end
xlabel("B\$_e\$ [a.u.]")
ylabel("PE signal [a.u.]")
xlim(minimum(wsXPS.Be),maximum(wsXPS.Be))
title("simulated spectra")
ylim(0.0,1.1maximum(IA_3))
