...
"""
function F_gaussian_un_val(z::Array{Cdouble,1},y::Array{Cdouble,1},ρ_val::Array{Cdouble,1},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},γval::Array{Cdouble,1})
    Nke = length(y);
    M = size(γ_H,2);
    M_d = length(γ_D);
    W_i = (1.0/4.0)./dropdims(sum(((1.0./γ).*γ_H).^2,dims=1),dims=1) # should be of dimension M
    sum(((z[1:Nke]-y)./γ).^2) + (1.0./γ)'*((γ_H.^2)*(z[Nke+1:Nke+M].^2)) + sum((1.0./γ_D.^2).*z[Nke+M+1:Nke+M+M_d].^2) + sum( ((z[Nke+M+M_d+1:end]-ρ_val)./γval).^2 )
end
...
"""
function G_gaussian_un_vals(x::Array{Cdouble,1})
    val = 0.0
    if any(x.<0.0)
        val = Inf
    end
...
"""
function F_convex_conjugate_gaussian_un_val(u::Array{Cdouble,1},y::Array{Cdouble,1},ρ_val::Array{Cdouble,1},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},γval::Array{Cdouble,1})
    Nke = length(y);
    M = size(γ_H,2);
    M_d = length(γ_D);
    W_i = (1.0/4.0)./dropdims(sum(((1.0./γ).*γ_H).^2,dims=1),dims=1) # should be of dimension M
    sum(u[1:Nke].*(y+0.25(γ.^2).*u[1:Nke])) + sum(W_i.*u[Nke+1:Nke+M]) + sum(0.25(γ_D.^2).*u[Nke+M+1:Nke+M+M_d]) + sum(u[Nke+M+M_d+1:end:end].*(ρ_val.+0.25*(γval.^2).*u[Nke+M+M_d+1:end:end]))
end

function G_convex_conjugate_gaussian_un_vals(u::Array{Cdouble,1})
    val = 0.0
    if any(u.>0.0)
        val = Inf
    end
...
"""
function prox_F_conj_gaussian_un_val(x::Array{Cdouble},y::Array{Cdouble,1},ρ_val::Array{Cdouble,1},σ::Cdouble,γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},γval::Array{Cdouble,1})
    Nke = length(y);
    M = size(γ_H,2);
    M_d = length(γ_D);
    ...
    u_all[Nke+M+M_d+1:end] = (x[Nke+M+M_d+1:end]-ρ_val)./(1.0.+0.5σ*(γval.^2))
...
"""
function prox_G_gaussian_un_vals(x::Array{Cdouble})
...
function alg2_cp_gaussian_un_val(x0::Array{Cdouble,1},s0::Array{Cdouble,1},y::Array{Cdouble,1},ρ_val::Array{Cdouble,1},A::Array{Cdouble,2},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},γval::Array{Cdouble,1},W_stop::Array{Cdouble,1};tau0::Cdouble=1.0,Niter::Int64=100,r_n_tol::Cdouble=1.0e-6,r_y_tol::Cdouble=0.5)
    ...
    for i in 1:Niter
        # dual step
        sn = prox_F_conj_gaussian_un_val(sn+sigman*A*xn_bar,y,ρ_val,sigman,γ,γ_H,γ_D,γval);
        # primal step
        xn_pre = xn; # memory (TODO: check that it does what I think it does)
        xn     = prox_G_gaussian_un_vals(xn-taun*A'*sn);
...
function alg2_cp_gaussian_un_no_mem_val(x0::Array{Cdouble,1},s0::Array{Cdouble,1},y::Array{Cdouble,1},ρ_val::Array{Cdouble,1},A::Array{Cdouble,2},γ::Array{Cdouble,1},γ_H::Array{Cdouble,2},γ_D::Array{Cdouble,1},γval::Array{Cdouble,1},W_stop::Array{Cdouble,1};tau0::Cdouble=1.0,Niter::Int64=100,r_n_tol::Cdouble=1.0e-6,r_y_tol::Cdouble=0.5)
    ...
    dX_W = copy(x0);
    F_ALL[1]      = F_gaussian_un_val(A*xn,y,ρ_val,γ,γ_H,γ_D,γval)
    G_ALL[1]      = G_gaussian_un_vals(xn);
    F_STAR_ALL[1] = F_convex_conjugate_gaussian_un_val(sn,y,ρ_val,γ,γ_H,γ_D,γval);
    G_STAR_ALL[1] = G_convex_conjugate_gaussian_un_vals(-A'*sn);
    ...
    for i in 1:Niter
        ...
        F_ALL[i+1]      = F_gaussian_un_val(A*xn,y,ρ_val,γ,γ_H,γ_D,γval)
        G_ALL[i+1]      = G_gaussian_un_vals(xn);
        F_STAR_ALL[i+1] = F_convex_conjugate_gaussian_un_val(sn,y,ρ_val,γ,γ_H,γ_D,γval);
        G_STAR_ALL[i+1] = G_convex_conjugate_gaussian_un_vals(-A'*sn);
