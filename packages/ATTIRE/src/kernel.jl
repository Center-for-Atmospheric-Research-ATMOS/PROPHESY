"""
    ...
    Dummy channel model centered on Ki [eV], with bandwidth ΔKi [eV] and gain T
    ...
    T is the linear gain; it depends on the collecting lens specs, the electron multiplier efficiency and the detector's efficiency

    ΔKi can be set as default value to a multiple of the analyzer's resolution, e.g. n*0.05, where n is 1, 2 or another integer, and 0.05 is the distance in eV between two consecutive channel's centers

    φi(Ke::Cdouble,Ki::Cdouble,ΔKi::Cdouble,T::Cdouble)

    Returns the gain of the analyzer's ith channel for a kinetic energy Ke. The channel's
